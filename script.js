...
window.onload = function () {
	...
	//カモネギくん画像
	const kamonegiImgUrl = "kamonegi.png";						//game.htmlからの相対パス
	game.preload([kamonegiImgUrl]);					//データを読み込んでおく
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		const mainScene = new Scene();					//シーン作成(Gameオブジェクトに貼り付けるSceneオブジェクト)
		game.pushScene(mainScene);  					//mainSceneシーンオブジェクトを画面に設置
		mainScene.backgroundColor = "#F5F5F5"; 			//mainSceneシーンの背景は黒くした

		//ゲーム画面左上のポイント表示テキスト
		const scoreText = new Label(); 					//テキストはLabelクラス
		scoreText.font = "20px Meiryo";				//フォントはメイリオ 20px 
		scoreText.color = 'rgba(128,128,0,1)';		//色　RGB+透明度　今回は白
		scoreText.width = 400;							//横幅指定　今回画面サイズ400pxなので、width:400pxだと折り返して二行目表示してくれる
		scoreText.moveTo(50, 30);						//移動位置指定
		mainScene.addChild(scoreText);					//mainSceneシーンにこの画像を埋め込む

		scoreText.text = "現在：" + point + "匹";					//テキストに文字表示 Pointは変数なので、ここの数字が増える

		//カモネギくんボタン
		const kamonegiImg = new Sprite(145, 150);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		kamonegiImg.moveTo(118, 100);						//カモネギ君ボタンの位置
		kamonegiImg.image = game.assets[kamonegiImgUrl];			//読み込む画像の相対パスを指定。　事前にgame.preloadしてないと呼び出せない
		mainScene.addChild(kamonegiImg);					//mainSceneにカモネギくん画像を貼り付ける  

		kamonegiImg.ontouchend = function () {				//カモネギ君ボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			point++;									//Pointを1増やす
			game.assets[clickSndUrl].clone().play();		//クリックの音を鳴らす。

			//クリックしたのでカモネギ君の画像のｘ位置を戻す
			this.x = -200;							//this.xって何？と思った方、Kamonegiの関数内でカモネギ君の座標を動かすときにはthisを使います。
		...
		game.onenterframe = function () {
			if (state == 0) { 							//state=0のとき、初期セット状態(Pointの状態を０にして)
				kamonegiImg.x = -200;						//カモネギ君のｘ座標を指定
				kamonegiImg.y = 100;						//カモネギ君のy座標を指定
				point = 0;  							//point初期化
			...
			if (state == 3) {							//状態３（point６以上から）
				kamonegiImg.x += 5;
				kamonegiImg.y = 200 + Math.sin(kamonegiImg.x / 70) * 100; // ｙ座標を振幅100pxのサイン波で移動(Sinは便利なので慣れとくといいよ！)
			}
			if (state == 4) {							//状態４（point９以上から）　4は初期セット状態（state=4）と移動状態（state=4.1)の2つに状態をわける		
				kamonegiImg.y = Math.random() * 400;			//ｙ座標の位置をランダムに決定
				state = 4.1;
			}
			if (state == 4.1) {							//状態４．１ 移動状態
				kamonegiImg.x += 10;						//ただ移動します
			}
			if (state == 5) {							//状態５（point１２以上から）　 ｙ軸が毎フレーム毎に変化する
				kamonegiImg.x += 20;						//移動します。
				kamonegiImg.y = Math.random() * 400;			//ｙ座標の位置を枚フレーム毎にランダム決定
			}
			...
			//ゲームオーバー判定
			if (kamonegiImg.x >= 400) {						//画面端にカモネギ君の画像が行ってしまったら
				game.popScene();					//mainSceneシーンを外す
				game.pushScene(endScene);				//endSceneシーンを読み込ませる

				//ゲームオーバー後のテキスト表示
				gameOverText.text = "GAMEOVER 記録：" + point + "匹";				//テキストに文字表示 
			}
		...
		//gameオブジェクトに貼り付ける、GAMEOVER（Sceneオブジェクト）
		...
		tweetBtn.ontouchend = function () {				//S_Tweetボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			...
			window.open("http://twitter.com/intent/tweet?text=頑張って" + point + "匹入手した&hashtags=ahoge&url=" + url); //ハッシュタグにahogeタグ付くようにした。
		};
